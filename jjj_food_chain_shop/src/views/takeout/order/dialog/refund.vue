  ...
  <div>
    <el-dialog title="退款" :visible.sync="dialogVisible" @close='dialogFormVisible' :close-on-click-modal="false"
      ...
      <el-form size="small" ref="form" :model="form">
        <el-form-item label="订单号" :label-width="formLabelWidth" prop="order_id"
          :rules="[{required: true,message: ' '}]">
          <el-input v-model="form.order_id" placeholder="请输入订单号" class="max-w460" :readonly="true"></el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.form.order_id = this.order_no;
    },
    methods: {
      /*处理*/
      submit() {
        ...
        self.$refs.form.validate((valid) => {
          if (valid) {
            this.$confirm('确认退款?', '提示', {
              ...
              self.loading = true;
              OrderApi.refund(form, true).then(data => {
                  self.loading = false;
                  self.$message({
                    message: data.msg,
                    type: 'success'
